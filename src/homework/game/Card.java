...
public enum Card {
	...
	public boolean activeCard(Player oneself, Player opponent, int index) {
//		activeCard: while (true) {
		switch (this.name) {
		case "뱅!":
			System.out.printf("%s(이)가 %s에게 뱅!\n", oneself.name, opponent.name);
		...
		case "강탈!":
			if (opponent.hand.length == 0) {
				...
				oneself.disCard(index);
				oneself.addCard(opponent.hand[randomIndex]);
				opponent.disCard(randomIndex);
				System.out.printf("%s(이)가 %s의 %s를 훔침\n", oneself.name,
						opponent.name, opponent.hand[randomIndex].name);
			}
			TimeUtil.secondsSleep(3);
			return true;
